        ...
        for j in range(0, len(object_list)):
            if ((j not in processed_objects)
                ...
                for k in range(0,len(dropbox_param)):
                    # For each pick and place operation, adjust the place_pose you send by a little bit so that your objects don't pile up 
                    # but land side by side nice and cozy inside the box.
                    if dropbox_param[k]['group'] == object_list_param[i]['group']:
                        place_pose.position.x = float(dropbox_param[k]['position'][0]) + random.random() / 10
                        place_pose.position.y = float(dropbox_param[k]['position'][1]) + random.random() / 10
                        place_pose.position.z = float(dropbox_param[k]['position'][2]) + random.random() / 10
